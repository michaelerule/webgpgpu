 ...
 * transforms) set to 1.
 *
 * question: if I set gl_PointSize does it automatically switch 
 * to point rasterizing? Hmm. 
 * 
...
function activateVertexShaderPosition(gl,program) {
    ...
    // Vector length of vertex shader argument
    // Since we pass an array of 2D points, this is 2
    // if we wanted to pass more, e.g. vec3, we'd set this to 3
    var vec_len = 2; 
    gl.enableVertexAttribArray(pointer);
    gl.vertexAttribPointer(
        pointer,  // index (pointer) to variable index (register)
        vec_len,  // size (it's a 2-vector / tuple)
 ...
 *      literal by adding a trailing decimal point
...
function asFloat(parameter) {
    ...
    if ((p^0)==p && (p.indexOf('.')<=-1)) {
        // is an integer and no decimal point
        p = p + '.0';
    }
    return '('+p+')';
    */
 ...
 * @param defines - optional list of #defines for the shader
 */
function getRasterProgram(gl,fragment,defines,included) {
    if (!$(fragment)) {
        console.log("Could not locate program "+fragment);
        return;
    }
    return buildRasterProgram(gl,$(fragment).text,defines,included);
}
 ...
 * surface.
 *
 * This is extremely useful as it allows using the output of rendering passes
 * as the input to other shader programs.
 * 
 * TODO: check for memeory leaks
 *
 * @param gl {WebGL context}
...
function newBasicFramebuffer(gl,params) {
    params = params||{};
    params['type']=gl.UNSIGNED_BYTE;
    console.log('Warning: overriding any type data, only uint8 textures supported');
    ...
    framebuffer.readPixels = function(x, y, width, height) {
...
function renderProgram(gl,program,framebuffer) {
    ...
    gl.useProgram(program);
    // This is important. Note the gl.TRIANGLES argument
    // This is the only place where we tell our shader that the points
    // we sent over define two triangles (that comprise the square "screen"
    // that lets us treat webGL like a 2D matrix engine). 
    // 
    // Here are other options that might be useful:
    //   gl.POINTS: Draws a single dot.
    //   gl.LINE_STRIP: Draws a straight line to the next vertex.
    //   gl.LINE_LOOP: "" ... and connects the last vertex back to the first.
    //   gl.LINES: Draws a line between a pair of vertices.
    //   gl.TRIANGLE_STRIP
    //   gl.TRIANGLE_FAN
    //   gl.TRIANGLES: Draws a triangle for a group of three vertices.
    //
    // Of interest is the gl.drawArrays(gl.POINTS, 0, particles.length);
    // command that would be used to render a particle system
    // 
...
function newParameter(gl,program,unif) {
    ...
    // this a pointer to the parameter in the shader
    ...
    if (tn('SAMPLER')) {
        ...
        param.mode    = 'texture';
        // setter function: takes texture object and texture unit
        // index.
        ...
        // function name needed to set variable
        param.setname = "uniform"+length+dtype+vector; 
        if (length>1 && vector!='v') {
            // A single 2,3, or 4 vector
            // these have different call signatures so we have to 
            // handle each case separately. 
            // TODO: raise warning if passed incompatible value
            if (length==2) {
...
function applyProgram(program,inputs,output) {
    ...
    var gl = program.gl;
    // Loop over all arguments (uniform variables)
    // and pass them to the GPU
    for (var key in inputs) if (inputs.hasOwnProperty(key)) {
        //console.log("locating "+key);
        // Retrieve a dictionary containing everything this shader program
        // knows about this parameter (its name, type, and a pointer with which
        // refer to it when sending data to the GPU)
        var param = getParameter(gl, program, key);
        // Make sure the parameter actually exists in the shader
        if (param == null) {
            console.log('Parameter '+key+' not in shader; '+
                        'was it compiled out because it is unused?');
            continue;
        }
        //console.log(param);
        // If it's a texture argument, then we pass it as a sampler
        // We have to explicitly count / keep track of which texture unit
        // we're using
        else if (param.mode === 'texture') {
            //console.log(
            //    'Parameter '+key+' is texture '+texture_counter);
            // look for an input matching the name of the shader paramter
            var texture = inputs[key];
            // Check: it is a texture that we've wrapped in a frameBuffer
            // object? If so extract the underlying texture object. 
            ...
            // Bind the texture to a texture unit; There are at least 8 of 
            // these slots available. We don't need to know the slot number,
            // WebGL will pass it as an argument for us. 
...
function getRasterGL(canvas) {
    ...
    // Question: how does WebGL know this is an array of 2D 
    // points defining two triangles? 
...
/** 
 * A simple copy program: can either move data from one texture to another
 * or draw a texture to the screen if no target argument is provided. 
 */ 
function GPUcopy(gl,W,H) {
    H = H || W || gl.height;
    W = W || gl.width;
    if (gl.hasOwnProperty('__copy__')) 
        if (gl.__copy__[W,H])
            return gl.__copy__;
    var __source__ = `
    ...
    var prog = buildRasterProgram(gl,__source__,{W:W,H:H});
    function gpu(d1,d2) {
        prog({data:d1},d2);
    };
    gpu.gl = gl;
