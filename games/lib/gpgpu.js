...
function asFloat(parameter) {
    return parameter; 
    // bypass
    // This was buggy and has been temporarily removed
    /*
...
function buildRasterProgram(gl,fragment,defines) {
    ...
    defines = defines||{};
    defines['W'] = defines['W'] || defines['width' ] || gl.width  || 256;
    defines['H'] = defines['H'] || defines['height'] || gl.height || 256;
    ...
    function callable_program(inputs,output) {
        ...
        //'Single input being treated as arguments list. Not as a render'+
        //' target. If this is not the case, missing variable errors will ensue.');
        //}
...
function newTexture(gl,params) {
    ...
    // unbind texture (dont working on it for now)
...
/** 
 * Generate a WebGL shader that performs a texture copy
 */
function GPUcopy(gl,options) {
    ...
    +"}\n";
    var prog = buildRasterProgram(gl,__source__,options);
    function gpu(d1,d2) {
        prog({data:d1},d2);
    };
    gpu.gl = gl;
